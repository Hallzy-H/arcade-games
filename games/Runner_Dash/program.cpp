...
#include <cstdlib> // this will be our randomizer.
...
struct
{
    ...
    bool game_over = false;     // True if all levels complete or player loses game
    bool game_won = false;      // True if all levels complete
    
} game_data;

void show_title_screen() // Harney - Our title screen, prompt's the player to start game.
{
    clear_screen(COLOR_BLACK);
    draw_text("Press SPACE to start!", COLOR_WHITE, font_named("default"), 25, 250, 430);
    if (key_typed(SPACE_KEY)) game_data.game_start = true;
}


void start_level()
{
    //how many levels will we include?
}
...
void end_level(bool successful) //Harney - When the level finishes, or player loses, give option to restart.
{
	if (successful)
	{
...
int main()
{
    open_window("Runner dash", 600, 800);
    window_toggle_border("Runner dash");
...
// Harney - Below is C# Code from Himanshu.
/*
using System;
